...
Add a script to the project's existing task runner. The user should run `pnpm run <prototype-name>` or equivalent — never need to remember a path.

### 6. Hand it over

Give the user the run command. The interesting moments are when they say "wait, that shouldn't be possible" — those are the bugs in the _idea_, which is the whole point.

### 7. Capture the answer

When the prototype has done its job, ask what it taught them. If not around, leave a `NOTES.md` next to the prototype.
